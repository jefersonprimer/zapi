...
import { listStores, listPromotions, type Store } from "@/lib/api";
...
function DeliveryCatalogPageContent({ cityParam }: { cityParam: string }) {
  const searchParams = useSearchParams();
  const [stores, setStores] = useState<Store[]>([]);
  const [promoStoreIds, setPromoStoreIds] = useState<Set<string>>(new Set());
  ...
  // Parse cityParam: might be "city=Frederico+Westphalen" or "frederico-westphalen" or "all"
  let selectedCity = decodeURIComponent(cityParam || "all");
  if (selectedCity.startsWith("city=")) {
    selectedCity = selectedCity.substring(5);
  }
    ...
    async function loadStores() {
      try {
        ...
        const [storesRes, promosRes] = await Promise.all([
          listStores(),
          listPromotions().catch(() => ({ promotions: [] })),
        ]);
        setStores(storesRes.stores || []);
        const promoIds = new Set(
          (promosRes.promotions || []).map((p) => p.store_id),
        );
        setPromoStoreIds(promoIds);
      } catch (err) {
  ...
  const processedStores = stores
    .filter((store) => {
      ...
      // Promotion filter: matches stores with promo products or has_promotions flag or has_coupons flag
      const matchesPromotion =
        !promotionOnly ||
        store.has_promotions === true ||
        store.has_coupons === true ||
        promoStoreIds.has(store.id);
...
export default function DeliveryCatalogPage({ params }: PageProps) {
  ...
  return (
    ...
    >
      <DeliveryCatalogPageContent cityParam={resolvedParams.city} />
    </Suspense>
  );
}
