...
// Standard emojis for the quick emoji picker
const EMOJIS = [
  "😀",
  "😂",
  "🥰",
...
export default function ConversasPage() {
    ...
    (newMsg: Message) => {
      setChats((prevChats) => {
        ...
        if (chatIndex > -1) {
          ...
          targetChat.last_message = newMsg.content || (newMsg.image_url ? (isAudioUrl(newMsg.image_url) ? "🎙️ Áudio" : "[Imagem]") : "[Mídia]");
  ...
  const startRecording = useCallback(async () => {
    ...
    try {
      ...
      alert("Erro ao acessar o microfone: Permissão negada ou não suportada no navegador.");
    }
  }, []);
  ...
  const sendAudioBlob = useCallback(
    async (audioBlob: Blob) => {
      ...
      try {
        ...
        const res = await sendMessage(token, selectedChat.id, "", uploadRes.url);
    ...
    [token, selectedChat, user, updateChatsListWithNewMessage, discardRecording]
  );
  ...
  const handleArchiveChatFromSidebar = async (chatId: string) => {
    ...
    try {
      ...
      setChats((prev) =>
        prev.map((c) => (c.id === chatId ? { ...c, is_archived: newArchived } : c))
      );
      if (selectedChat?.id === chatId) {
        setSelectedChat((prev) => (prev ? { ...prev, is_archived: newArchived } : prev));
      }
    } catch (err) {
  ...
  const handlePinChatFromSidebar = (chatId: string) => {
    setChats((prev) =>
      prev.map((c) => (c.id === chatId ? { ...c, is_pinned: !c.is_pinned } : c))
    );
    if (selectedChat?.id === chatId) {
      setSelectedChat((prev) => (prev ? { ...prev, is_pinned: !prev.is_pinned } : prev));
    }
  };
  ...
  const handleFavoriteChatFromSidebar = async (chatId: string) => {
    ...
    try {
      ...
      if (selectedChat?.id === chatId) {
        setSelectedChat((prev) => (prev ? { ...prev, is_favorite: newFav } : prev));
      }
    } catch (err) {
      console.error("Error toggling favorite:", err);
    }
  ...
  const handleClearChatFromSidebar = async (chatId: string) => {
    ...
    try {
      ...
      setChats((prev) =>
        prev.map((c) => (c.id === chatId ? { ...c, last_message: null, last_message_at: null } : c))
      );
    } catch (err) {
      console.error("Error clearing chat:", err);
    }
        ...
        <div className="flex-1 flex flex-col h-full bg-background relative overflow-hidden">
          {selectedChat ? (
            <>
              ...
              {/* Message Input Box */}
              <div className="p-3 pb-5 border-t border-card-border bg-card-bg/60 backdrop-blur-md">
                ...
                {isRecording || recordedUri ? (
                  ...
                  <form
                    ...
                    className="flex items-center gap-1.5 relative bg-neutral-100 dark:bg-neutral-800/60 border border-card-border rounded-2xl p-1.5 pl-3 transition-all focus-within:border-neutral-400 dark:focus-within:border-neutral-600 focus-within:ring-1 focus-within:ring-neutral-400 dark:focus-within:ring-neutral-600"
                  >
                    <input
                      type="file"
                      ref={fileInputRef}
                    ...
                    {!inputText.trim() && !selectedFile ? (
                      <button
                        ...
                        className="p-2.5 bg-emerald-600 hover:bg-emerald-700 text-white rounded-xl transition-all cursor-pointer flex-shrink-0 shadow-md active:scale-95"
                        title="Gravar áudio"
                      >
                      ...
                      <button
                        ...
                        className="p-2.5 bg-neutral-900 text-white dark:bg-white dark:text-neutral-900 rounded-xl hover:opacity-90 disabled:opacity-40 disabled:cursor-not-allowed transition-all cursor-pointer flex-shrink-0 shadow-md"
                      >
                        {uploadingFile ? (
                    ...
                    {showEmojiPicker && (
                      <div className="absolute bottom-16 left-2 z-50">
                        <EmojiGifStickerPicker
                          ...
                          onSelectGif={(gifUrl) => handleSendDirectMedia(gifUrl)}
