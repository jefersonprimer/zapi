...
export default function CartScreen() {
  ...
  const { storeName, items, removeItem, updateQuantity, getSubtotal, getTotal, discount, couponCode, clearCart } =
    useCartStore();
  ...
  return (
    <View style={[styles.container, { backgroundColor: colors.background }]}>
      ...
      <View style={[styles.storeBanner, { backgroundColor: colors.surface }]}>
        <Text style={[styles.storeLabel, { color: colors.textSecondary }]}>Loja:</Text>
      ...
      <FlatList
        ...
        renderItem={({ item }) => {
          ...
          return (
            <View style={[styles.itemCard, { backgroundColor: colors.cardBackground, borderColor: colors.border }]}>
              <View style={styles.itemInfo}>
                ...
                {item.addons.length > 0 && (
                  <View style={styles.addonsContainer}>
                    {item.addons.map((addon) => (
                      <Text key={addon.id} style={[styles.addonText, { color: colors.textSecondary }]}>
                ...
                {item.observation ? (
                  <Text style={[styles.itemObs, { color: colors.textSecondary }]} numberOfLines={1}>
      ...
      >
        <View style={styles.totalRow}>
          ...
          <Text style={[styles.totalValue, { color: colors.text }]}>R$ {subtotal.toFixed(2)}</Text>
        </View>
        ...
        <View style={[styles.totalRow, { marginTop: 4 }]}>
          <Text style={[styles.totalLabel, { color: colors.text, fontWeight: "700", fontSize: 16 }]}>Total</Text>
          <Text style={[styles.totalValue, { color: colors.tint, fontSize: 18, fontWeight: "700" }]}>R$ {total.toFixed(2)}</Text>
        </View>
        <TouchableOpacity
